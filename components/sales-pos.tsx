...
function getLinePricing(line: SaleLine) {
  ...
  const discount =
    isPercentDiscount(line.discountType)
...
export function SalesPos() {
  ...
  const subtotalTtc = pricing.reduce(
    (sum, item) => sum + item.lineSubtotalTtc,
    0
  )
  const lineDiscountTotal = pricing.reduce(
    (sum, item) => sum + item.discount,
    0
  )
  const totalAfterLineDiscounts = Math.max(0, subtotalTtc - lineDiscountTotal)
  const globalDiscount =
    isPercentDiscount(globalDiscountType)
  ...
  const hasValidLines = lines.some(
    (line) => line.productName && line.quantity > 0
  )
  const hasErrors = lines.some(
    (line) =>
      (line.productName && line.quantity <= 0) ||
      (!line.productName && line.quantity > 0)
  )
                  ...
                  <TableRow key={line.id}>
                    <TableCell className="min-w-64">
                      ...
                      >
                        <ComboboxInput
                          ...
                          aria-invalid={
                            showValidation && !line.productName
                          }
                        >
                          <InputGroupAddon
                            align="inline-end"
                    ...
                    <TableCell className="w-28">
                      <Input
                        ...
                        aria-invalid={
                          showValidation && line.quantity <= 0
                        }
                        onChange={(event) =>
                          updateLine(line.id, {
                    ...
                    <TableCell className="w-40">
                      <ButtonGroup className="w-full">
                        ...
                        >
                          <SelectTrigger
                            className={`${dropdownTriggerBaseClassName} w-24`}
                          >
                            <SelectValue>
                              {(value) =>
                                getDiscountLabel(
                                  (value as DiscountType) ?? "percent"
                                )
                              }
                            </SelectValue>
                          </SelectTrigger>
                        ...
                        <Input
                          ...
                          onChange={(event) =>
                            updateLine(line.id, {
                              discountValue: parseNumericInput(
                                event.target.value
                              ),
                            })
                          }
              ...
              <div className="space-y-2">
                ...
                <ButtonGroup className="w-full">
                  <Select
                    ...
                    onValueChange={(value) =>
                      setGlobalDiscountType(value as DiscountType)
                    }
                  >
                    <SelectTrigger
                      className={`${dropdownTriggerBaseClassName} w-24`}
      ...
      <CardFooter className="justify-end">
        {showValidation ? (
          <p className="text-destructive mr-4 text-xs">
            Ajoutez un produit et une quantité supérieure à 0 avant
            d&apos;enregistrer.
          </p>
        ) : null}
