...
function ExpandableTable({ className, multiple = true, children, ...props }: ExpandableTableProps) {
  return (
    <div
    ...
    >
      <AccordionRoot
        ...
        render={(rootProps) => (
          <table {...rootProps} data-slot="table" className={rootProps.className} />
        )}
      >
        {children}
      </AccordionRoot>
...
function ExpandableTableRow({
  ...
  return (
    <AccordionItemPlain value={value} render={<tbody />}>
      <AccordionHeader render={<TableRow className={cn(isLast && "border-b-0", rowClassName)} />}>
        {row}
      </AccordionHeader>
      <TableRow className={cn("bg-muted/40 border-0", isLast && "border-b-0", panelRowClassName)}>
...
function ExpandableTableTrigger({ label, className }: ExpandableTableTriggerProps) {
  return (
    <AccordionTriggerButton
